...
export default function Page() {
  ...
  useEffect(() => {
    setIsClient(true)
    const authenticated = sessionStorage.getItem('worksplus-authenticated') === 'true'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        sessionStorage.setItem('worksplus-authenticated', 'true')
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="relateive">
            ...
            <button 
              type="submit"
              disabled={isLoading}
              className="absolute right-0 top-1/2 -translate-y-1/2 px-6 py-2 bg-gray-200 hover:text-gray-900 disabled:opacity-50 transition-colors"
            >
              送信
            </button>
          </div>
